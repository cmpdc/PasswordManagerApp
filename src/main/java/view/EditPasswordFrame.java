...
public class EditPasswordFrame extends Stage implements Initializable {
    // basically the same as GeneratePasswordFrame but for editing... duh!
    ...
    private Password passwordToEdit;
    private String generatedSalt;
    private OffsetDateTime currentDateTime;
    private int passwordLength;
    private String encodedPassword;
    private final PasswordGenerator passwordGenerator = new PasswordGenerator(25,true,true,true,true);
    private String generatedPassword = passwordGenerator.generatePassword();
    private PasswordChecker passwordChecker;

    private String decodedPasswordToEdit;
    ...
    public EditPasswordFrame(){
        ...
        this.setTitle(Constants.APP_NAME + " - EDIT PASSWORD");
        this.initModality(Modality.APPLICATION_MODAL);
        this.initStyle(StageStyle.TRANSPARENT);
    }

    // this is for passing data from "EditPasswordFrame" controller
    public Password getEditedPassword(){return this.newPassword;}
    public boolean isSaveButtonPressed(){return this.isSaveButtonPressed;}
    public boolean isNewPassword(){return this.isNewPassword;}

    @Override
    public void initialize(URL url, ResourceBundle resourceBundle) {
        // NEEDED! This is to receive data from another controller!
        this.passwordToEdit = PasswordHolder.getInstance().getPassword();

        this.encodedPassword = PasswordHolder.getInstance().getPassword().getEncodedPassword();
        this.generatedSalt = PasswordHolder.getInstance().getPassword().getGeneratedSalt();
        this.currentDateTime = PasswordHolder.getInstance().getPassword().getDateAndTimeRaw();

        this.decodedPasswordToEdit = passwordGenerator.decryptPassword(
                PasswordHolder.getInstance().getPassword().getEncodedPassword(),
                this.masterKey,
                PasswordHolder.getInstance().getPassword().getGeneratedSalt()
        );

        this.passwordLength = decodedPasswordToEdit.length();

        this.passwordChecker = new PasswordChecker(this.decodedPasswordToEdit);
        // ---------------------------------------------------------- //
        ...
        generatedPasswordField.setText(decodedPasswordToEdit);
        passwordStrengthBar.setProgress(passwordChecker.getPasswordStrengthScore());

        passwordTitle.setText(passwordToEdit.getTitle());
        notes.setText(passwordToEdit.getNotes());
    ...
    public void saveButtonFunc(MouseEvent mouseEvent) {
        ...
        if(mouseButton == MouseButton.PRIMARY){
            ...
            PasswordHolder.getInstance().setPassword(this.newPassword);
